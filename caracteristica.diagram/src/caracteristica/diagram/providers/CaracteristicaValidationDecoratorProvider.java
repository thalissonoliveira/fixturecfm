 ...
 * @generated
 */
public class CaracteristicaValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	 ...
	 * @generated
	 */
	private static final String MARKER_TYPE = CaracteristicaDiagramEditorPlugin.ID + ".diagnostic"; //$NON-NLS-1$
	...
	public boolean provides(IOperation operation) {
		...
		return view != null && LPSEditPart.MODEL_ID.equals(CaracteristicaVisualIDRegistry.getModelID(view));
	}
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					...
					CaracteristicaDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
	...
	public static class StatusDecorator extends AbstractDecorator {
		...
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				CaracteristicaDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
		...
		public void refresh() {
			...
			try {
				...
				CaracteristicaDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
	...
	static class MarkerObserver implements IFileObserver {
		...
		private String getType(IMarker marker) {
			try {
				...
				CaracteristicaDiagramEditorPlugin.getInstance().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
